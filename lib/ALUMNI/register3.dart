...
class MyRegister3 extends StatefulWidget {
  const MyRegister3({Key? key}) : super(key: key);

  @override
  _MyRegister3State createState() => _MyRegister3State();
}

class _MyRegister3State extends State<MyRegister3> {
  ...
  Future<void> _registerUser() async {
    ...
    setState(() {
      ...
      if (usn.isEmpty)
        usnError = "Enter ID";
      else if (!usn.toLowerCase().contains("dt") &&
          !usn.toLowerCase().contains("DT") &&
    ...
    if (nameError == null &&
      ...
      try {
        ...
        // await user?.updateProfile(displayName: usn);

        // dummy=usnController.text;
        // print("retain original usn");
        // print(dummy);
        if (user != null) {
          // Add user details to Firestore
          await FirebaseFirestore.instance.collection('alumni_account_information').doc(userCredential.user!.uid).set({
            'name': name,
            'id': usn,
            'email': email,
            'password':password,
