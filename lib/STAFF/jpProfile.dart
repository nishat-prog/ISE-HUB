...
class JPProfile extends StatefulWidget {
  const JPProfile({Key? key}) : super(key: key);
  @override
  JPProfileState createState() => JPProfileState();
}

class JPProfileState extends State<JPProfile> {
  ...
  Future<String> _uploadFile(dynamic file,String foldername, String fileName) async {
    Reference storageReference = FirebaseStorage.instance.ref().child('STAFF/Journal_papers/$foldername/$fileName');
    if (file is String) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Journal Papers",
            style:TextStyle(
                color:Colors.white,
                fontWeight: FontWeight.bold
            )),
...
// Photo upload field
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: () async{
                                String eventId = await _addJPDetails(
